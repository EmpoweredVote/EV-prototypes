...
import {
  acquirePoliticianLock, releasePoliticianLock,
  approvePoliticianReview, rejectPolitician, editAndResubmitPolitician
} from '../../api/sheets'
...
} from '../ManageMode/politicianConstants'

function PoliticianReviewForm() {
  ...
  const [loading, setLoading] = useState(!location.state?.politician)
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState(null)
  const [lockAcquired, setLockAcquired] = useState(false)

  useEffect(() => {
    if (!location.state?.politician) {
      loadData()
    } else {
      populateForm(location.state.politician)
      tryAcquireLock(politicianId)
    }

    return () => {
      if (lockAcquired) {
        releasePoliticianLock(politicianId).catch(() => {})
      }
    }
  }, [politicianId])
  ...
  const loadData = async () => {
    try {
      ...
      setPolitician(found)
      populateForm(found)
      await tryAcquireLock(politicianId)
    } catch (err) {
  ...
  const tryAcquireLock = async (id) => {
    try {
      await acquirePoliticianLock(id)
  ...
  const handleApprove = async () => {
    try {
      setSaving(true)
      setError(null)
      await approvePoliticianReview(politicianId)
      navigate('/review/politicians')
    } catch (err) {
      setError(err.message)
    } finally {
      setSaving(false)
  ...
  const handleReject = async () => {
    try {
      setSaving(true)
      setError(null)
      await rejectPolitician(politicianId, rejectComment)
      navigate('/review/politicians')
    } catch (err) {
      setError(err.message)
    } finally {
  ...
  const handleEditAndResubmit = async () => {
    ...
    try {
      setSaving(true)
      setError(null)
      await editAndResubmitPolitician(politicianId, {
  ...
  if (!politician) {
    return (
      <div className="error-container">
        ...
        <Link to="/review/politicians" className="btn-secondary">Back to Queue</Link>
      </div>
    )
  }
  ...
  return (
    <div className="review-form">
      <div className="page-header">
        <div>
          <h1>Review: {politician.full_name}</h1>
      ...
      <div className="review-content">
        <div className="review-meta-info">
          <p><strong>Author:</strong> {politician.added_by}</p>
          {politician.review_count > 0 && (
            <p><strong>Current approvals:</strong> {politician.review_count}/2</p>
          )}
        ...
        {!isEditing && !showRejectForm ? (
          ...
          <div className="reject-form">
            ...
            <div className="form-group">
              ...
              <textarea
                ...
                placeholder="Explain why this politician entry should be rejected..."
                rows={3}
              />
            </div>
            <div className="form-actions">
              <button onClick={() => setShowRejectForm(false)} className="btn-secondary" disabled={saving}>Cancel</button>
              <button onClick={handleReject} className="btn-primary btn-reject" disabled={saving}>
          ...
          <div className="form-content">
            <p className="edit-notice">You are editing this politician entry. Submitting will make you the new author and reset review progress.</p>
...
export default PoliticianReviewForm
